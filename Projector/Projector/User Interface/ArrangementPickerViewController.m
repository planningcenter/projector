//
//  ArrangementPickerViewController.m
//  Projector
//
...
@interface ArrangementPickerViewController ()

@property (nonatomic, weak) PCOTableView *tableView;

@end

@implementation ArrangementPickerViewController
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    return [[self.song orderedArrangements] count];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.textLabel.text = [[[self.song orderedArrangements] objectAtIndex:indexPath.row] name];
    return cell;
}

- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    PCOArrangement *arrangement = [[self.song orderedArrangements] objectAtIndex:indexPath.row];
    [self.selectedItem setSelectedArrangement:arrangement];
    if ([[arrangement orderedKeys] count] > 1) {
